...
- **TUI、WebUI 与 Desktop 全端专家团**
  - 会话可通过 `--expert <id>` 或 TUI `/expert list|show|bind|unbind|switch` 命令绑定可复用专家包。团队包会注入 lead 身份与名册并自动启用成员调度；单人包只改变 lead 身份。
  - 替换已绑定专家会创建分叉，而不会覆盖源会话。WebUI 专家面板和 Desktop ACP 的 **Expert** 选项复用同一条 Runtime 绑定与分叉路径。
  - 成员生命周期投影现在携带成员名称、emoji、角色和专家身份，供 TUI、WebUI 与 Desktop 卡片展示。成员完成会在 lead 的边界投递，绝不会自行启动新 run。
...
- 专家团：覆盖 Runtime 绑定/分叉、命名成员事件、TUI 与 Serve 的“成员终态不直接开 run”守卫、ACP bind/fork 进程路径、Desktop 投影与跨入口 ESM 空闲 gate。
