// SVG 图标（stroke 风格，取自 workbuddy 原型的图标精灵）。
// renderer 不用 <symbol> 全局精灵，直接内联，避免 loadFile 下的引用问题。
...
const PATHS: Record<string, string> = {
  ...
  search: '<circle cx="10.5" cy="10.5" r="7"/><path d="M21 21l-4.3-4.3"/>',
  ...
  clip: '<path d="M21.4 11l-9.2 9.2a6 6 0 0 1-8.5-8.5l9.2-9.2a4 4 0 0 1 5.7 5.7l-9.2 9.2a2 2 0 0 1-2.9-2.9l8.5-8.4"/>',
  slash: '<path d="M16 4L8 20"/>',
  image: '<rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/>',
  shield: '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',
  sliders: '<path d="M4 21v-7M4 10V3M12 21v-9M12 8V3M20 21v-5M20 12V3M1 14h6M9 8h6M17 16h6"/>',
  list: '<path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01"/>',
  globe: '<circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>',
  channels: '<path d="M21 15a3 3 0 0 1-3 3h-2.5a3.5 3.5 0 0 1-3.5-3.5V8a2.5 2.5 0 0 0-2.5-2.5H6"/><circle cx="18" cy="15" r="3"/><circle cx="10" cy="15" r="3"/><circle cx="18" cy="15" r="1"/><circle cx="10" cy="15" r="1"/>',
...
// Keep every Desktop surface on the same brand asset as the packaged app.
// build.ts copies this image beside index.html in the renderer bundle.
export function createMothxLogo(): HTMLImageElement {
...
// hydrateIcons 替换所有 <span class="wi" data-icon="name"> 为内联 SVG。
export function hydrateIcons(root: ParentNode = document): void {
  root.querySelectorAll<HTMLElement>('.wi[data-icon]').forEach((el) => {
    if (el.dataset.hydrated === '1') return;
    el.dataset.hydrated = '1';
    el.innerHTML = iconSVG(el.dataset.icon || 'doc');
  });
}
