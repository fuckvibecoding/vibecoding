...
const runtimePanel = await readFile(new URL('../views/settings/RuntimePanel.tsx', import.meta.url), 'utf8');
...
test('diagnostic logs never touch localStorage or the Desktop store', () => {
  assert.doesNotMatch(runtimePanel, /localStorage\s*[\[.]/i, 'diagnostic logs must not be persisted in localStorage');
  assert.doesNotMatch(runtimePanel, /desktop\.storeSet\([^)]*diagnostic/i, 'diagnostic logs must not be persisted in the Desktop store');
  assert.doesNotMatch(runtimePanel, /desktop\.storeSet\([^)]*log/i, 'log filter or entries must not enter the Desktop store');
});

test('diagnostic log rendering uses JSX text nodes and does not insert HTML', () => {
  assert.doesNotMatch(runtimePanel, /dangerouslySetInnerHTML/, 'log rendering must not insert raw HTML');
  assert.doesNotMatch(runtimePanel, /innerHTML\s*=/, 'log rendering must not assign innerHTML');
  assert.match(runtimePanel, /\{entry\.message\}/, 'log messages must render as escaped text nodes');
});

test('diagnostic log viewer has a filter, refresh, bounded merge, and empty state', () => {
  assert.match(runtimePanel, /settings\.diagnosticLogsFilter/, 'log viewer needs a filter input');
  assert.match(runtimePanel, /getDiagnosticLogs\(\)/, 'log viewer needs a refresh path');
  assert.match(runtimePanel, /mergeDiagnosticLogs/, 'log viewer must bound and dedupe through the shared merge helper');
  assert.match(runtimePanel, /settings\.diagnosticLogsEmpty/, 'log viewer needs an empty state');
  assert.match(runtimePanel, /settings\.diagnosticLogsCount/, 'log viewer needs a count display');
});
...
test('diagnostic log code does not reference session working directory state', () => {
  ...
  assert.doesNotMatch(runtimePanel, /activeSessionCwd|newSessionCwd/, 'diagnostic log code must not reference per-session cwd state');
});
