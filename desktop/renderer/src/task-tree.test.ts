...
test('task tree keeps the newest loaded sessions first without using a directory filter', () => {
  const sessions = [
    { sessionId: 'older', cwd: '/a', provider: '', model: '', updatedAt: '2026-09-05T08:00:00Z' },
    { sessionId: 'newer', cwd: '/b', provider: '', model: '', updatedAt: '2026-09-06T08:00:00Z' },
  ];
  assert.deepEqual(sortTaskSessions(sessions).map((entry) => entry.sessionId), ['newer', 'older']);
});
